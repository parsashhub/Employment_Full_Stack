...
export const createFormikObjects = (data: any) => {
  const initialValues = {};
  const validationObj = {};
  data.map((field: any) => {
...
export const mapCreateElement = ({ type, ...rest }, formik: any) => {
    ...
    return (
      ...
      <form className="w-full" onSubmit={formik?.handleSubmit}>
        <Grid container spacing={config?.spacing ?? 2}>
          {config?.inputData?.map((field: any) => {
            if (field?.type === "Empty") {
...
export const typeOnlyNumber = (event: any) => {
