...
const initialState: initialStateProps = user?.data?.settings
  ? {
      initial: user.data.settings,
      defaults: _.merge({}, user.data.settings),
      current: _.merge({}, user.data.settings),
    }
  : {
...
export const fuseSettingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(setDefaultSettings.fulfilled, (state, { payload }) => {
        return payload;
      })
